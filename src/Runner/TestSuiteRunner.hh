...
class TestSuiteRunner {
  ...
  public static function run(string $filePath) : ?TestSuite {
    ...
    $runner = new TestSuiteRunner($filePath, $className);
    $runner->loadClass();
  ...
  public function loadClass() : void {
    ...
    $this->testFound = TestSuiteRunner::isTest($class);

    if ($this->testFound) {
      $allMethods = $class->getMethods();
      foreach($allMethods as $method) {
        if (TestSuiteRunner::isTestMethod($method)) {
          $this->testMethods[$method->getName()] = $method;
        } else if (TestSuiteRunner::isLifecycleMethod($method)) {
  ...
  private static function isLifecycleMethod(\ReflectionMethod $method) : bool {
    if (!$method->isPublic() || !array_key_exists($method->getName(), TestSuiteRunner::$RESERVED_NAMES)) {
      return false;
    }
    $reservedMethod = TestSuiteRunner::$RESERVED_NAMES[$method->getName()];
